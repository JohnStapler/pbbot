...
async def LB(ctx):
    ...
    for i, (wpm, user_id, quote) in enumerate(results[:10], start=1):
        ...
        leaderboard_message += f"{i}. {username}: **{formatted_wpm} WPM** - Quote: {quote}\n"
...
async def typing_test(ctx):
    ...
    if ctx.author.guild_permissions.administrator or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        ...
        try:
            ...
            await ctx.send("Kunde inte tysta användaren. Kontrollera att boten har rätt behörigheter och att den har en högre roll än användaren.")
            return
    ...
    embed = discord.Embed(title="3...", description=f"**{quote}**", color=0xFF0000)  # Röd för 3
    ...
    if not (ctx.author.guild_permissions.administrator or ctx.author.top_role.position > ctx.guild.me.top_role.position):
        try:
            ...
            await ctx.send("Kunde inte återställa användarens skrivbehörighet. Kontrollera att boten har rätt behörigheter.")
            return
        except discord.HTTPException:
